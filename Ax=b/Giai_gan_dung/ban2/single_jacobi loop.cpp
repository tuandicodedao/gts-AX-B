...
template<typename T>
bool checkRow(const matrix<T> &A) // ham kiem tra xem ma tran co troi hay khong(bao gom ca dung va khong dung vi tri)
{
    arr.resize(A.row + 1);
    vector<int> use;
    use.resize(A.row + 1);
    for(int i = 0; i < use.size(); i++) use[i] = 0;
    double norm1;
    double Max;
    int k = -1;
...
template<typename T>
bool checkCol(const matrix<T> &A) //ham kiem tra xem ma tran co troi hay khong(bao gom ca dung va khong dung vi tri)
{
    return checkRow(!A);
}
template<typename T>
int dominant(const matrix<T> &A)//ham kiem tra tinh troi cua ma tran
{
    ...
    if (check)  return 2;
    if(checkRow(A)) return 3;
    if(checkCol(A))  return 4;
    return 0;
}
template<typename T>
...
template<typename T>
matrix<T> loop(matrix<T> &A, matrix<T> &B, matrix<T> &X0, double &eps, int &type, double &q, int &normType, double w = 1)//ham lap
{
    fstream wi;
    ...
    case 1:
    {
        ...
        do
        {
            ...
            wi << fixed << setprecision(-log10(eps) + 9) << X;

			//cout << "Ket qua cua vong lap thu " << loopNumber  << endl;
            //cout << fixed << setprecision(-log10(eps) + 9) << X;
    ...
    case 2:
    {
        ...
        for (int i = 1; i <= loopNumber; ++i)
        {
            ...
            wi << fixed << setprecision(-log10(eps) + 9) << X;
			//cout << "Ket qua vong lap thu " << i << endl;
            //cout << fixed << setprecision(-log10(eps) + 5) << X;
        }
        cout << "So vong lap la : " << loopNumber << "\n";
        cout << q << endl;
...
matrix<T> jacobiloop(matrix<T> A, matrix<T> B, matrix<T> &X0, double &eps) // lap jacobi
{
    ...
    if(Domi == 2)
    {
      ...
      for (int i = 0; i < A.row; ++i)
        ...
        double t1 = fabs(A[0][0]);
        double t2 = fabs(A[0][0]);
        for (int i = 1; i < A.col; i++)
        {
            t1 = max(t1, fabs(A[i][i]));
            t2 = min(t2, fabs(A[i][i]));
        }
        w = t1 / t2;
    }
...
int main(){
    ...
    if (select == 1)
    {
        ...
        cout << "Ket qua cua A * X -B la : " << endl;
        wi << setprecision(-log10(eps) + 9) << (A * X - B);
        cout << setprecision(-log10(eps) + 11) << (A * X - B);
    }
    else
    {
        cout << "Giai pt AX=B " << endl;
        ...
        cout << "ket qua cua A * X - B la : " << endl;
        cout << setprecision(-log10(eps) + 5) << (A * X - B);
    }
    wi.close();
    re.close();
    return 0;
